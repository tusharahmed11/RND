...
class ShimmerRibbonView : ShimmerFrameLayout,
    RibbonInterface {
    ...
    private fun getAttrs(attributeSet: AttributeSet, defStyleAttr: Int) {
        val typedArray =
            context.obtainStyledAttributes(attributeSet,
                R.styleable.ShimmerRibbonView, defStyleAttr, 0)
        try {
            setTypeArray(typedArray)
        } finally {
            typedArray.recycle()
    ...
    private fun setTypeArray(typeArray: TypedArray) {
        ribbon = ribbonView(context) {
            ribbonDrawable =
                typeArray.getDrawable(R.styleable.ShimmerRibbonView_shimmer_ribbon_drawable)
            ...
            paddingTop =
                typeArray.getDimension(
                    R.styleable.ShimmerRibbonView_shimmer_ribbon_padding_top,
                    paddingTop
                )
            paddingRight =
                typeArray.getDimension(
